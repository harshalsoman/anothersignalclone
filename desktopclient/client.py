...
class Application(tkinter.Tk):
    ...
    def add_contact(self):
        ...
        self.conn.send(f"!addcontact:{self.contact}".encode())
    ...
    def clientchat(self):
        while not self.should_quit:                                                 #If we are not in the 'quit' state then do :
            try:
                ...
                if data.startswith("!notifycontact"):
                    notifier = data.split(':')[1]
                    tkinter.messagebox.showinfo("Contact Notification", f"{notifier} has added you as a contact")
